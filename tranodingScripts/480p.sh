...
$FFMPEG/ffmpeg -i $newfile -r 25  \
	...
	-vf "yadif=0, scale=852:trunc(ow/a/2)*2" -hls_flags independent_segments \
	-crf 20 -b:v 1200k -maxrate 1200k -bufsize 1200k -refs 3 -profile:v high -bf 2 -g 30 -force_key_frames "expr:gte(t,n_forced*3)" -f segment -segment_time 3 -segment_list $MAIN_DIR/$filebase-480p/$filebase-480p.m3u8  $MAIN_DIR/$filebase-480p/"%04d".480p.ts
#	> log/$filebase-480p.log 2>&1 ;
#end480p=$(date)
#COMMAND FOR 5 SECONDNS TS SEGMENTS -hls_time 1
#$FFMPEG/ffmpeg -i $newfile -deinterlace -vf "yadif=0, scale=852:trunc(ow/a/2)*2" \
...
#  -b:v 1200k -maxrate 1200k -bufsize 1200k -force_key_frames "expr:gte(t,n_forced*1)" -async 1 \
#   -hls_time 1 -hls_playlist_type vod -hls_segment_filename $MAIN_DIR/$filebase-480p/"%04d".480p.ts $MAIN_DIR/$filebase-480p/$filebase-480p.m3u8


cd $MAIN_DIR/$filebase-480p/
...
#   sed -i "s/$fil/$TEMN.ts/g" $filebase-480p.m3u8
#   mv "$fil" "$TEMN.ts"
# done

#rm  $filebase-480p.m3u81
#while read line;
#do
#  if [[ $line == *".ts" ]];
#    then
...
#              #TEMN="$(radn)$USCOUNTER$(myfunc)"
#
#        echo $TEMN.ts  >>  $filebase-480p.m3u81
#        mv "$line" "$TEMN.ts"
#    else
#        echo $line >>  $filebase-480p.m3u81
#    fi
#
#
#
#   # do something with $line here
#done <$filebase-480p.m3u8
#
#mv $filebase-480p.m3u81 $filebase-480p.m3u8

cd $folder
# create variant m3u8 - ugly way
echo "#EXT-X-STREAM-INF:PROGRAM-ID=1,BANDWIDTH=1200000,RESOLUTION=852x480,NAME=\"480p\"" >>  $MAIN_DIR/$filebase.m3u8
